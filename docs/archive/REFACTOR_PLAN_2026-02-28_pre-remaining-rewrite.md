...
This document is the implementation plan for the post-fix refactor audit described in the archived run/verify doc at `docs/archive/RUN_VERIFY_FIXES.md`, section `G) Refactor and maintainability (NO behavior change)`.
...
These rules apply whenever a refactor slice removes, replaces, inlines, or archives code:
...
5. Maintain a removal ledger in this file during the run.
   - append entries under a dedicated section rather than relying on memory
...
### Oversized File Inventory (Top 15)

| # | File | Lines | Why it grew | Severity |
|---|------|------:|-------------|----------|
| 1 | `apps/web/app/globals.css` | 2457 | Single monolith CSS file for all pages, components, and utilities. Mixed concerns: sidebar, graph, chat, KB, theme, buttons, modals, animations. | 🔴 Critical |
...
| 5 | `apps/web/app/tutor/page.tsx` | 707 | Entire tutor chat page in one component — state management, message rendering, concept switch UI, composer, progress bars, phase indicators. | 🟡 High |
...
| 14 | `apps/web/app/kb/page.tsx` | 467 | Knowledge base page — upload, table, empty state, status badges, delete. | 🟢 Low |
...
- `npm --prefix apps/web test`: now 45 tests across 7 files
...
5. Make future follow-up fixes from the archived run/verify work cheaper to implement.
...
- preserve the current clean typecheck state and prevent regressions in:
  - `apps/web/app/tutor/page.tsx`
  - `apps/web/components/concept-graph.tsx`
...
- the archived run/verify doc already points to document lifecycle truthfulness and async ingestion as ongoing concerns
...
- No new refactor slices are required after the latest `RUN_VERIFY_FIXES` work.
- The current plan still fits the repository shape.
- The only plan updates required were:
  - source-doc reference moved to `docs/archive/RUN_VERIFY_FIXES.md`
  - `R0` now protects the clean typecheck state rather than fixing known type errors
  - removal/reversal documentation is now required inside this file
...
> Do NOT implement these changes as part of this refactor plan. This is a documentation-only audit.
> Implementation should happen after the refactor slices are complete and the codebase is stable.

### Responsive Layout Breakpoints (currently missing)
...
This file is both the stable roadmap and the execution journal.

Update this file in-place as slices are completed. Use the Verification Block Template above to record outcomes for each slice.

Previously, `docs/REFACTOR.md` served as a separate audit log. Its unique content (file inventory, mobile readiness checklist) has been merged into this file. `docs/REFACTOR.md` is no longer maintained.

## Unified Refactor Prompt

Use this single prompt to start or resume the refactor run:
...
Begin with the current slice in execution order. If starting fresh, begin with slice R0 (Baseline and Guardrails) exactly as described.
