...
`conversation_summaries` are generated by the Phase 13 LLM summarizer through an owned service. They summarize only visible user/assistant turns that have fallen outside the raw recent-turn window; hidden tool turns and provider reasoning are excluded. The tutor reads learner state and conversation summaries before raw long history when available.
