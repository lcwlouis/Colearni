...
async def test_upload_creates_private_source_record(api_client, db_engine):
    ...
    assert data["metadata_json"]["ingestion_status"] == "pending_parse"
...
async def test_upload_persists_revision_provenance_and_private_object(
    ...
    assert response_revision["parser_name"] == "none"
    assert response_revision["parser_version"] == "upload-only-v1"
    assert response_revision["status"] == "pending_parse"
    assert response_revision["file_size_bytes"] == len(content)
    assert response_revision["metadata_json"]["parsing"] == "deferred"

    async with _sessionmaker(db_engine)() as session:
        persisted_revision = await session.get(
            SourceRevision, uuid.UUID(response_revision["id"])
        )
...
async def test_source_revision_append_only_keys_are_unique(db_engine, tmp_path):
    ...
    async with _sessionmaker(db_engine)() as session:
        ...
        assert revision.status == "pending_parse"
...
async def test_source_read_does_not_expose_raw_private_text(api_client, db_engine):
    ...
    resp = await api_client.get(
        f"/api/workspaces/{workspace_id}/sources/{upload.json()['id']}"
    )
