...
| Mode | When to select |
|------|---------------|
| `socratic` | Default mode. The learner is engaging with the concept normally — answering questions, exploring, or continuing the dialogue without a special trigger. |
| `direct` | The learner explicitly asks for an explanation or answer. Triggers include: "explain", "just tell me", "what is", "give me the answer", "summarize", "give an example", "how does X work". |
...
1. If `direct` and `repair` both seem applicable, prefer `repair` — fixing a misconception is more important than re-explaining.
2. If the learner's message is short and ambiguous (e.g. "ok", "I see", "hmm"), choose `socratic`.
...
4. Use the conversation summary and recent turns for context, but base the mode decision primarily on the latest learner message (provided as the next user message).
