...
class TestExecuteTopicPlanDataFlow:
    ...
    def test_plan_queries_become_candidates(self, mock_rdb: MagicMock) -> None:
        """build_query_plan output is transformed into candidate dicts for enqueue."""
        ...
        with patch("domain.research.query_planner.build_query_plan", return_value=plan):
            with patch("domain.research.query_planner.enqueue_query_results") as mock_enqueue:
                ...
                # Verify enqueue received correct structure
                ...
                assert results_arg[0]["source_url"].startswith("planned://web/")
                assert results_arg[1]["source_url"].startswith("planned://academic/")
                assert "gradient descent" in results_arg[0]["title"]
