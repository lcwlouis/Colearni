...
class TestDocumentSummaryAsset:
    ...
    def test_asset_enforces_length_bound(self, registry: PromptRegistry) -> None:
        rendered = registry.render("document_document_summary_v1", {
            "chunks": "some text",
        })
        assert "500" in rendered or "2 or 3 sentences" in rendered

    def test_asset_grounded_no_outside_knowledge(self, registry: PromptRegistry) -> None:
        rendered = registry.render("document_document_summary_v1", {
            "chunks": "some text",
        })
    ...
    def test_asset_plain_text_output(self, registry: PromptRegistry) -> None:
        rendered = registry.render("document_document_summary_v1", {
            "chunks": "some text",
        })
